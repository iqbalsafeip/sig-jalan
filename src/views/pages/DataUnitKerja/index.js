...
const fields = ['no','nama','keterangan','is_active', 'show_details'];

const DataUnitKerja = props => {
    ...
    const deleteData = id => {
      Swal.fire({
        ...
        if(res.isConfirmed){
          dispatch(_deleteData({id: id, pathAPI: 'unit_kerja', actionType: 'DELETE_UNIT_KERJA'})).then(()=>{
            Swal.fire({
              title: 'Berhasil',
    ...
    React.useEffect(()=> {
      dispatch(getAll({pathAPI : 'unit_kerja', actionType: 'SET_UNIT_KERJA'})).then(res => {
        console.log(res);
      }).catch(err => {
          ...
          <CCard>
            <CCardHeader >
              ...
              <div className='card-header-actions'>
                <Link className="btn btn-primary" to="unit-kerja/tambah"  >
                    Tambah Data
                </Link>
              </div>
            </CCardHeader>
            <CCardBody>
            <CDataTable
              items={state.organisasi.unit_kerja}
              fields={fields}
              itemsPerPage={5}
              pagination
                tableFilter
                    ...
                    (item, index)=>{
                        return (
                            ...
                            <CCollapse show={details.includes(index)}>
                                ...
                                <Link className="btn btn-warning " to={`unit-kerja/update/${item.id}`} >
                                    Update
                                </Link>
