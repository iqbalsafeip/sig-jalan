...
const fields = ['no','klasifikasi','keterangan', 'is_active', 'show_details']

const KlasifikasiDisposisi = () => {
    ...
    const onCreate = (klasifikasi, keterangan, is_active, cb) => {
        dispatch(createData({pathAPI: 'klasifikasi_disposisi', actionType: 'CREATE_KLASIFIKASI_DISPOSISI', data: {
    ...
    const onUpdate = (klasifikasi, keterangan, is_active, cb) => {
        dispatch(updateData({pathAPI: 'klasifikasi_disposisi', actionType: 'UPDATE_KLASIFIKASI_DISPOSISI', id: currData , data: {
            klasifikasi: klasifikasi,
            keterangan: keterangan,
            is_active: is_active
        }})).then(res => {
    ...
    const onDelete = (id, index) => {
        dispatch(deleteData({pathAPI: 'klasifikasi_disposisi', actionType: 'DELETE_KLASIFIKASI_DISPOSISI', data: {
            id: id
        }})).then(res => {
            console.log(res);
            toggleDetails(index)
    ...
    React.useEffect(()=> {
        dispatch(getAll({ pathAPI : 'klasifikasi_disposisi', actionType: 'SET_KLASIFIKASI_DISPOSISI' })).then((res)=>{
  ...
  return (
    <>
      <CRow>
        <CCol xs="12" lg="12">
          <CCard>
            ...
            <CCardBody>
            <CDataTable
              items={dataMaster.klasifikasi_disposisi}
              fields={fields}
              itemsPerPage={5}
              pagination
                tableFilter
                  ...
                  'details':
                    (item, index)=>{
                        return (
                            ...
                            <KlasifikasiDisposisi.ModalAlert show={modalAlert[index] ? modalAlert[index].shownModal : false} onClick={()=> onDelete(item.id, index)} toggle={()=> toggleModalAlert(index)} disabled={dataMaster.is_loading} />
                            </>
                        )
                    },
                    'is_active' : 
      ...
      <KlasifikasiDisposisi.Modal modalShown={modalTambah} toggle={()=> setModalTambah(!modalTambah)} onSubmit={onCreate} disabled={dataMaster.is_loading} />
      <KlasifikasiDisposisi.Modal modalShown={modalUpdate} toggle={()=> setModalUpdate(!modalUpdate)} onSubmit={onUpdate} disabled={dataMaster.is_loading} isUpdate data={dataMaster.klasifikasi_disposisi.filter(d => d.id == currData)[0]} />
...
const Modal = props => {
    ...
    React.useEffect(()=> {
        if(props.isUpdate && props.modalShown) {
            const {klasifikasi, keterangan, is_active } = props.data
    ...
    return (
      ...
      >
        ...
        <CModalBody>
            <CFormGroup>
                <CLabel htmlFor="nf-nama">Nama klasifikasi</CLabel>
                <CInput
                    type="text"
                    value={klasifikasi}
        ...
        <CModalFooter>
          <CButton color="primary" onClick={()=> props.onSubmit(klasifikasi,keterangan,is_active, cb)} disabled={props.disabled} > {props.disabled ? <CSpinner size='sm' /> : null} {props.isUpdate ? 'Update' : 'Tambah'}</CButton>{' '}
          <CButton
            color="secondary"
...
KlasifikasiDisposisi.ModalAlert = props => {
    return (
        <CModal
            show={props.show}
            onClose={props.toggle}
...
KlasifikasiDisposisi.Modal = Modal

export default KlasifikasiDisposisi
