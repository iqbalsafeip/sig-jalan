...
const KlasifikasiSurat = () => {
    ...
    const onCreate = (klasifikasi, keterangan, is_active, cb) => {
        dispatch(createData({pathAPI: 'klasifikasi_surat', actionType: 'CREATE_KLASIFIKASI_SURAT', data: {
    ...
    const onUpdate = (klasifikasi, keterangan, is_active, cb) => {
        dispatch(updateData({pathAPI: 'klasifikasi_surat', actionType: 'UPDATE_KLASIFIKASI_SURAT', id: currData , data: {
            klasifikasi: klasifikasi,
            keterangan: keterangan,
            is_active: is_active
        }})).then(res => {
    ...
    const onDelete = (id, index) => {
        dispatch(deleteData({pathAPI: 'klasifikasi_surat', actionType: 'DELETE_KLASIFIKASI_SURAT', data: {
            id: id
        }})).then(res => {
            console.log(res);
            toggleDetails(index)
    ...
    React.useEffect(()=> {
        dispatch(getAll({ pathAPI : 'klasifikasi_surat', actionType: 'SET_KLASIFIKASI_SURAT' })).then((res)=>{
  ...
  return (
    <>
      <CRow>
        <CCol xs="12" lg="12">
          <CCard>
            ...
            <CCardBody>
            <CDataTable
              items={dataMaster.klasifikasi_surat}
              fields={fields}
              itemsPerPage={5}
              pagination
                tableFilter
                  ...
                  'details':
                    (item, index)=>{
                        return (
                            ...
                            <KlasifikasiSurat.ModalAlert show={modalAlert[index] ? modalAlert[index].shownModal : false} onClick={()=> onDelete(item.id, index)} toggle={()=> toggleModalAlert(index)} disabled={dataMaster.is_loading} />
                            </>
                        )
                    },
                    'is_active' : 
      ...
      <KlasifikasiSurat.Modal modalShown={modalTambah} toggle={()=> setModalTambah(!modalTambah)} onSubmit={onCreate} disabled={dataMaster.is_loading} />
      <KlasifikasiSurat.Modal modalShown={modalUpdate} toggle={()=> setModalUpdate(!modalUpdate)} onSubmit={onUpdate} disabled={dataMaster.is_loading} isUpdate data={dataMaster.klasifikasi_surat.filter(d => d.id == currData)[0]} />
...
KlasifikasiSurat.ModalAlert = props => {
    return (
        <CModal
            show={props.show}
            onClose={props.toggle}
...
KlasifikasiSurat.Modal = Modal

export default KlasifikasiSurat
