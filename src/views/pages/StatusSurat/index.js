...
const fields = ['no','status','keterangan', 'is_active', 'show_details']
...
const StatusSurat = () => {
    ...
    const onCreate = (status, keterangan, is_active, cb) => {
        dispatch(createData({pathAPI: 'status_surat', actionType: 'CREATE_STATUS_SURAT', data: {
            status: status,
    ...
    const onUpdate = (status, keterangan, is_active, cb) => {
        dispatch(updateData({pathAPI: 'status_surat', actionType: 'UPDATE_STATUS_SURAT', id: currData , data: {
            status: status,
            keterangan: keterangan,
            is_active: is_active
        }})).then(res => {
    ...
    const onDelete = (id, index) => {
        dispatch(deleteData({pathAPI: 'status_surat', actionType: 'DELETE_STATUS_SURAT', data: {
            id: id
        }})).then(res => {
            console.log(res);
            toggleDetails(index)
    ...
    React.useEffect(()=> {
        dispatch(getAll({ pathAPI : 'status_surat', actionType: 'SET_STATUS_SURAT' })).then((res)=>{
            console.log('awdawd : ',res.data.data);
  ...
  return (
    <>
                  ...
                  'details':
                    (item, index)=>{
                        return (
                            ...
                            <StatusSurat.ModalAlert show={modalAlert[index] ? modalAlert[index].shownModal : false} onClick={()=> onDelete(item.id, index)} toggle={()=> toggleModalAlert(index)} disabled={dataMaster.is_loading} />
                            </>
                        )
                    },
                    'is_active' : 
      ...
      <StatusSurat.Modal modalShown={modalTambah} toggle={()=> setModalTambah(!modalTambah)} onSubmit={onCreate} disabled={dataMaster.is_loading} />
      <StatusSurat.Modal modalShown={modalUpdate} toggle={()=> setModalUpdate(!modalUpdate)} onSubmit={onUpdate} disabled={dataMaster.is_loading} isUpdate data={dataMaster.status_surat.filter(d => d.id == currData)[0]} />
...
const Modal = props => {
    ...
    React.useEffect(()=> {
        if(props.isUpdate && props.modalShown) {
            const {status, keterangan, is_active } = props.data
            setStatus(status);
    ...
    return (
      ...
      >
        ...
        <CModalBody>
            <CFormGroup>
                <CLabel htmlFor="nf-nama">Nama status</CLabel>
                <CInput
                    type="text"
                    value={status}
        ...
        <CModalFooter>
          <CButton color="primary" onClick={()=> props.onSubmit(status,keterangan,is_active, cb)} disabled={props.disabled} > {props.disabled ? <CSpinner size='sm' /> : null} {props.isUpdate ? 'Update' : 'Tambah'}</CButton>{' '}
          <CButton
            color="secondary"
